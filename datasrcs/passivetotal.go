...
func NewPassiveTotal(sys systems.System) *PassiveTotal {
	...
	pt.BaseService = *requests.NewBaseService(pt, "PassiveTotal")
	return pt
}
...
func (pt *PassiveTotal) OnStart() error {
	...
	pt.API = pt.sys.Config().GetAPIKey(pt.String())
	if pt.API == nil || pt.API.Username == "" || pt.API.Key == "" {
		pt.sys.Config().Log.Printf("%s: API key data was not provided", pt.String())
	}
...
func (pt *PassiveTotal) OnDNSRequest(ctx context.Context, req *requests.DNSRequest) {
	...
	pt.CheckRateLimit()
	bus.Publish(requests.SetActiveTopic, eventbus.PriorityCritical, pt.String())
	bus.Publish(requests.LogTopic, eventbus.PriorityHigh,
		fmt.Sprintf("Querying %s for %s subdomains", pt.String(), req.Domain))

	url := pt.restURL(req.Domain)
	headers := map[string]string{"Content-Type": "application/json"}
	page, err := http.RequestWebPage(url, nil, headers, pt.API.Username, pt.API.Key)
	if err != nil {
		bus.Publish(requests.LogTopic, eventbus.PriorityHigh, fmt.Sprintf("%s: %s: %v", pt.String(), url, err))
		return
	}
