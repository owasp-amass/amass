...
func NewNetcraft(sys systems.System) *Netcraft {
	n := &Netcraft{SourceType: requests.SCRAPE}

	n.BaseService = *requests.NewBaseService(n, "Netcraft")
	return n
}
...
func (n *Netcraft) OnDNSRequest(ctx context.Context, req *requests.DNSRequest) {
	...
	bus.Publish(requests.LogTopic, eventbus.PriorityHigh,
		fmt.Sprintf("Querying %s for %s subdomains", n.String(), req.Domain))

	url := n.getURL(req.Domain)
	page, err := http.RequestWebPage(url, nil, nil, "", "")
	if err != nil {
		bus.Publish(requests.LogTopic, eventbus.PriorityHigh, fmt.Sprintf("%s: %s: %v", n.String(), url, err))
		return
	}
