...
func NewVirusTotal(sys systems.System) *VirusTotal {
	...
	v.BaseService = *requests.NewBaseService(v, "VirusTotal")
	return v
}
...
func (v *VirusTotal) OnStart() error {
	...
	v.API = v.sys.Config().GetAPIKey(v.String())
	if v.API == nil || v.API.Key == "" {
		v.haveAPIKey = false
		v.sys.Config().Log.Printf("%s: API key data was not provided", v.String())
	}
