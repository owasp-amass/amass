...
func NewExalead(sys systems.System) *Exalead {
	e := &Exalead{SourceType: requests.SCRAPE}

	e.BaseService = *requests.NewBaseService(e, "Exalead")
	return e
}
...
func (e *Exalead) OnDNSRequest(ctx context.Context, req *requests.DNSRequest) {
	...
	if err != nil {
		bus.Publish(requests.LogTopic, eventbus.PriorityHigh, fmt.Sprintf("%s: %s: %v", e.String(), url, err))
		return
	}
