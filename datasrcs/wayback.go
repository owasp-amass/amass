...
func NewWayback(sys systems.System) *Wayback {
	w := &Wayback{
	...
	w.BaseService = *requests.NewBaseService(w, "Wayback")
	return w
}
...
func (w *Wayback) OnDNSRequest(ctx context.Context, req *requests.DNSRequest) {
	...
	if err != nil {
		bus.Publish(requests.LogTopic, eventbus.PriorityHigh, fmt.Sprintf("%s: %v", w.String(), err))
		return
	}
