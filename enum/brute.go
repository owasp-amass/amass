...
type AlterationsManager struct {
	...
	inFilter  *stringset.StringFilter
	outFilter *stringset.StringFilter
	altState  *alts.State
}
...
func NewAlterationsManager(e *Enumeration) *AlterationsManager {
	am := &AlterationsManager{
		...
		inFilter:  stringset.NewStringFilter(),
		outFilter: stringset.NewStringFilter(),
...
func (r *AlterationsManager) InputName(req *requests.DNSRequest) {
	...
	if !r.enum.Config.Alterations || r.inFilter.Duplicate(req.Name) {
		return
	}
...
func (r *AlterationsManager) Stop() error {
	...
	r.inFilter = stringset.NewStringFilter()
	r.outFilter = stringset.NewStringFilter()
...
func (r *AlterationsManager) generateAlts() int {
	...
	for name := range names {
		if !r.enum.Config.IsDomainInScope(name) || r.outFilter.Duplicate(name) {
			continue
		}
...
func (r *GuessManager) OutputNames(num int) []*requests.DNSRequest {
	...
	for name := range guesses {
		...
		if domain == "" || name == "" || r.outFilter.Duplicate(name) {
			continue
		}
