...
func (t *txtServiceDiscovery) check(e *et.Event) error {
	...
	// The engine provides FQDN assets as *oamdns.FQDN
	fqdn, ok := e.Entity.Asset.(*oamdns.FQDN)
	if !ok {
		return nil
	}

	// Determine cache TTL boundary; assetType and recordType are literal strings
	since, err := support.TTLStartTime(e.Session.Config(), "FQDN", "FQDN", t.name)
	if err != nil {
		return err
	}

	// Gather TXT entries from live DNS and cache
	var entries []string
	for _, rr := range support.Query(e.Session, fqdn, dns.TypeTXT) {
	...
	for _, prop := range support.LoadDNSRecordProperties(e.Session, fqdn, t.source, since) {
